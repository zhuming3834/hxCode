...
export default {
  ...
  methods: {
    ...
    loginBtnClick: function(){
      ...
      var tempWx = self.$utils.getWxDate();
      var openID = '';
      var userName = '';
      var userPhoto = '';
      if (tempWx != '') {
        openID = tempWx.openid;
        userName = tempWx.nickname;
        userPhoto = tempWx.headimgurl;
      }
      self.$axios.get('user/user/loginAndNew.do',{
        params: {
          phone: self.tel,
          validateCode: self.yzm,
          openID: openID,  // 微信openid
          userName: userName,// 微信昵称
          userPhoto: userPhoto, // 微信头像
        }
      }).then(function (res){
        // alert(1);
        if (res.data.ret_code == 0) {
